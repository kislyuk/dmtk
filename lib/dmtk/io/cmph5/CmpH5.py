...
class CmpH5( CmpH5Group, InfoTableContainer ):
    ...
    def __del__( self ):
        try:
            ...
            pass # h5py object must have already been closed
    ...
    def close( self ):
        """Calls close on the underlying H5 file."""
        if self._items != None:
            self.closeAll()
        if self._cache != None:
    ...
    def alnHitIterator( self, loadSequence=True, computeAlnPosTranslations=False, auxTables=pulseTables ):
        ...
        for refGroup in self.refGroupIterator():
            for alnHit in refGroup.alnHitIterator( loadSequence, computeAlnPosTranslations, auxTables ):
                yield alnHit
...
class SFCmpH5( CmpH5 ):
    ...
    def _setVersion( self ):
        self._group.attrs['Version'] = V1_2_0_SF
...
class PBCmpH5( SFCmpH5 ):
    ...
    def getPlsPaths( self ):
        """Returns a set of paths to the pls files in this cmp.h5"""
        # TODO: Move this class to _dmtk and import our path resolution code to replace this. -drw
        plsPaths = set( )
        for row in self["/MovieInfo"].asRecArray( ):
            plsGlob = "/mnt/data*/vol*/%07d/%04d/%s/%s.pls.h5" % ( row["Exp"], row["Run"], self.reportsFolder, row["Name"] )
            plsPaths |= set( glob( plsGlob ) )
        return plsPaths
